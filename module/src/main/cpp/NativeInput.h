...
inline static ImGuiKey KeyCodeToImGuiKey(int32_t key_code) {
    switch (key_code) {
    case AKEYCODE_TAB:
        return ImGuiKey_Tab;
...
inline jmethodID MotionEvent_getX, MotionEvent_getY, MotionEvent_getAction, MotionEvent_getToolType, MotionEvent_getButtonState, MotionEvent_getAxisValue;
inline jmethodID KeyEvent_getUnicodeChar, KeyEvent_getMetaState, KeyEvent_getAction, KeyEvent_getKeyCode, KeyEvent_getScanCode;

inline jboolean (*orig_NativeInjectEnvent)(JNIEnv *env, jobject __this, jobject input_event);
inline jboolean hook_NativeInjectEvent(JNIEnv *env, jobject __this, jobject input_event) {
    if (ImGui::GetCurrentContext() == nullptr) return orig_NativeInjectEnvent(env, __this, input_event);
    ...
    if (env->IsInstanceOf(input_event, motion_event)) {
        ...
        // ImGui_ImplAndroid_HandleInputEvent(x, y, action);
    ...
    return orig_NativeInjectEnvent(env, __this, input_event);
}
